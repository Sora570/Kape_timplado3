...
function updateOrderStats() {
    // Update order statistics in settings
    const totalOrders = orders.pending ? orders.pending.length + orders.completed.length : 0;
    const pendingOrders = orders.pending ? orders.pending.length : 0;
    const completedOrders = orders.completed ? orders.completed.length : 0;
    
    // You can add order stats display here
    console.log(`Orders: ${totalOrders} total, ${pendingOrders} pending, ${completedOrders} completed`);
}
...
// On page load
// Initial data loading is now handled in DOMContentLoaded event
loadOrders();
